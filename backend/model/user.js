        ...
        return new Promise((resolve, reject) => {
            bcrypt.hash(values[2], saltRounds, (err, hash) => {
                ...
                pool.query(`Insert INTO user(username, email, password, role) values (?,?,?,?)`, values, (error, result) => {
                    if (error) {
                        if (error.code === 'ER_DUP_ENTRY') reject({ status: 403, error: `Email ${values[1]} is already in use` })
                    }
    ...
    userLogin: (data) => {
        return new Promise((resolve, reject) => {
            pool.query(`Select * from user where email =?`, data.email, (error, result) => {
                if (error) reject({ status: 500, error: error })
                if (result.length === 0) reject({ status: 404, error: 'Email not found' })
                bcrypt.compare(data.password, result[0].password, (err, valid) => {
                    if (err) reject({ status: 500, error: err })
                    if (!valid) reject({ status: 401, error: 'Incorrect password' })
                    let sql3 = `Insert into token(token, fk_user_id) values (?,?)`
                    pool.query(`Select * from token where fk_user_id = ?`, result[0].id, (error, result2) => {
                        if (error) reject({ status: 500, error: error })
                        if (result2.length !== 0) reject({status: 403, error: 'User is already logged in'})
                        pool.query(sql3, [uuidv4(), result[0].id], (error, result) => {
                            if (error) reject({ status: 500, error: error })
    ...
    userLogout: (data) => {
        return new Promise((resolve, reject) => {
            pool.query(`Delete from token where fk_user_id = ?`, data.fk_user_id, (error, result) => {
                if (error) reject({ status: 500, error: error })
                if (result.affectedRows === 0) reject({ status: 404, error: 'User not found' })
